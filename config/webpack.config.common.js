...
module.exports = {
    ...
    optimization: {
        splitChunks: {
            chunks: 'all', // 一般可以指定三种形式 all（全部的 chunk，包含所有类型的 chunk）、async（按需加载的 chunk） 和 initial（初始的 chunk）
            ...
            cacheGroups: {
                ...
                default: { // 默认缓存组的配置
                    ...
                    minChunks: 2,
                    priority: -20,
                    reuseExistingChunk: true // 是否重用 chunk，如果当前块包含已经从主bundle中分离出来的模块，那么它将被重用，而不是生成一个新的模块，一般设置为 true
                }
            }
        },
        runtimeChunk: {
